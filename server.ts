...
const SUPABASE_KEY =
  ...
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtsY2ZkYnN2bHhmZW56YnpsdHZxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI4MTYxOTIsImV4cCI6MjA5ODM5MjE5Mn0.fvRTEg_6G-XJEo2QG_uj0sKqAUsJN7N_qYDWDOExfUY";
...
async function startServer() {
  ...
  const authenticateUser = async (
    ...
    const { data: profile, error } = await db
      .from("profiles")
      .select("*")
      .or(`id.eq.${token},email.ilike.${token}`)
      .single();
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      const policyId = `POL-${new Date().getFullYear()}-${Date.now().toString().slice(-6)}`;
      ...
      const cost =
        freq === "annually" ? targetPlan.premium_amount * 12
        : freq === "quarterly" ? targetPlan.premium_amount * 3
        : targetPlan.premium_amount;
  ...
  app.get("/api/members", authenticateUser, async (req, res) => {
    try {
      ...
      const { data: members } = await db
        .from("profiles")
        .select("*, policies(id, plan_id, status, end_date, plans(name))")
        .eq("role", "member");
  ...
  app.put("/api/members/:id", authenticateUser, async (req, res) => {
    try {
      ...
      if (requester.role === "member" && id !== requester.id) {
        res.status(403).json({ error: "Unauthorised profile edit" }); return;
      }
  ...
  app.post("/api/members/:id/beneficiary", authenticateUser, async (req, res) => {
    try {
      ...
      if (!name || !relationship || !dob) {
        res.status(400).json({ error: "name, relationship, dob required" }); return;
      }
      if (requester.role === "member" && id !== requester.id) {
        res.status(403).json({ error: "Unauthorised" }); return;
      }

      const { data: policy } = await db.from("policies").select("id").eq("user_id", id).eq("status", "active").maybeSingle();
      if (!policy) { res.status(400).json({ error: "No active policy found" }); return; }

      const { data: newBen, error } = await db.from("beneficiaries").insert({
        policy_id: policy.id, name, relationship, dob, national_id: national_id || null,
      }).select().single();
      if (error) { res.status(500).json({ error: error.message }); return; }
  ...
  app.post("/api/plans", authenticateUser, async (req, res) => {
    try {
      const requester = (req as any).user;
      if (!["admin", "staff"].includes(requester.role)) {
        res.status(403).json({ error: "Admin/Staff only" }); return;
      }
      const { name, description, premium_amount, coverage_limit, benefits, exclusions } = req.body;
      if (!name || !description || premium_amount === undefined || !coverage_limit) {
        res.status(400).json({ error: "Missing plan fields" }); return;
      }
      const planId = `plan-${Date.now()}`;
      const { data: plan, error } = await db.from("plans").insert({
        id: planId, name, description,
  ...
  app.put("/api/plans/:id", authenticateUser, async (req, res) => {
    try {
      ...
      if (premium_amount !== undefined) updates.premium_amount = Number(premium_amount);
      if (coverage_limit !== undefined) updates.coverage_limit = Number(coverage_limit);
  ...
  app.get("/api/policies", authenticateUser, async (req, res) => {
    try {
      ...
      const { data } = await db
        .from("policies")
        .select("*, profiles(name,email,national_id), plans(name,coverage_limit)")
  ...
  app.post("/api/policies/:id/renew", authenticateUser, async (req, res) => {
    try {
      ...
      if (!pol) { res.status(404).json({ error: "Policy not found" }); return; }
  ...
  app.get("/api/claims", authenticateUser, async (req, res) => {
    try {
      ...
      let query = db
        .from("claims")
        .select("*, policies(user_id, plan_id, profiles(name,national_id)), providers(name)")
        .order("date_filed", { ascending: false });

      if (requester.role === "member") {
        const { data: userPolicies } = await db.from("policies").select("id").eq("user_id", requester.id);
        const ids = (userPolicies || []).map((p: any) => p.id);
      ...
      if (provider) query = query.eq("provider_id", provider as string);
      if (policy) query = query.eq("policy_id", policy as string);
  ...
  app.post("/api/claims", authenticateUser, async (req, res) => {
    try {
      ...
      if (!policy_id || !diagnosis || !treatment || amount_claimed === undefined) {
      ...
      if (!targetPolicy) { res.status(404).json({ error: "Policy not found" }); return; }
      if (targetPolicy.status === "suspended") {
        res.status(400).json({ error: "Policy SUSPENDED. Clear overdue premiums first." }); return;
      }
      ...
      const { data: duplicate } = await db.from("claims")
        .select("id")
        .eq("policy_id", policy_id)
        .ilike("diagnosis", diagnosis.trim())
        .gte("date_filed", twoHoursAgo)
        .maybeSingle();
      ...
      if (!autoFlagged && settings?.allow_auto_approval && requestedSum <= (settings?.low_claim_threshold || 500000)) {
      ...
      const { data: newClaim, error } = await db.from("claims").insert({
        ...
        review_notes: initialStatus === "approved"
          ? "SLA auto-approved (low-value threshold)."
          : autoFlagged ? flagReason : "Submitted for staff review.",
      ...
      const auditAction = autoFlagged ? "CLAIM_FLAGGED_SUSPICIOUS"
        : initialStatus === "approved" ? "CLAIM_AUTO_APPROVED" : "CLAIM_SUBMITTED";
      await writeAudit(requester.id, requester.name, auditAction, "claims", newClaim.id);

      const notifMsg = autoFlagged
        ? `ALERT: Claim for ${diagnosis} flagged. ${flagReason}`
        : initialStatus === "approved"
          ? `Claim for ${diagnosis} (UGX ${requestedSum.toLocaleString()}) AUTO-APPROVED!`
          : `Claim ${newClaim.id} submitted and under review.`;
  ...
  app.put("/api/claims/:id/review", authenticateUser, async (req, res) => {
    try {
      ...
      const updates: any = { status, reviewer_id: requester.id, review_notes: notes };
      if (status === "approved" || status === "paid") {
        updates.amount_approved = amount_approved !== undefined ? Number(amount_approved) : undefined;
      } else {
  ...
  app.post("/api/claims/:id/pay", authenticateUser, async (req, res) => {
    try {
      ...
      if (existing.status !== "approved") { res.status(400).json({ error: "Claim must be APPROVED first" }); return; }
  ...
  app.get("/api/premiums", authenticateUser, async (req, res) => {
    try {
      const requester = (req as any).user;
      let query = db.from("premiums")
        .select("*, policies(user_id, plan_id, profiles(name), plans(name))")
  ...
  app.post("/api/premiums/:id/pay", authenticateUser, async (req, res) => {
    try {
      ...
      if (!premium) { res.status(404).json({ error: "Premium not found" }); return; }
      if (premium.status === "paid") { res.status(400).json({ error: "Already paid" }); return; }
      if (requester.role === "member" && premium.policies?.user_id !== requester.id) {
        res.status(403).json({ error: "Unauthorised" }); return;
      }

      const receiptNo = `REC-${Date.now()}`;
      await db.from("premiums").update({ status: "paid", paid_date: new Date().toISOString(), receipt_number: receiptNo }).eq("id", id);
    ...
    try {
      ...
      for (const pr of outstanding || []) {
        if (pr.policies?.user_id) {
          await addNotification(pr.policies.user_id,
  ...
  app.post("/api/providers", authenticateUser, async (req, res) => {
    try {
      const requester = (req as any).user;
      if (!["admin", "staff"].includes(requester.role)) {
        res.status(403).json({ error: "Staff/Admin only" }); return;
      }
      const { name, type, location, contact, approved_plans } = req.body;
      if (!name || !type || !location || !contact) {
        res.status(400).json({ error: "name, type, location, contact required" }); return;
      }
  ...
  app.put("/api/providers/:id", authenticateUser, async (req, res) => {
    try {
      ...
      if (error) { res.status(500).json({ error: error.message }); return; }
      await writeAudit(requester.id, requester.name,
  ...
  app.post("/api/notifications/:id/read", authenticateUser, async (req, res) => {
    try {
      ...
      if (n.user_id !== requester.id) { res.status(403).json({ error: "Unauthorised" }); return; }
  ...
  app.get("/api/settings", authenticateUser, async (req, res) => {
    try {
      ...
      res.json({
        ...
        lowClaimThreshold: data?.low_claim_threshold ?? 500000,
  ...
  app.get("/api/analytics", authenticateUser, async (req, res) => {
    try {
      ...
      const totalPremiumRevenue = (premiums || [])
  ...
  app.post("/api/ai/chat", authenticateUser, async (req, res) => {
    try {
      ...
      if (!apiKey) {
        res.status(500).json({ error: "Gemini API key not configured" }); return;
      }
      ...
      const planSummary = (plans || []).map((p: any) =>
        `- ${p.name}: UGX ${(p.premium_amount || 0).toLocaleString()}/mo, limit UGX ${(p.coverage_limit || 0).toLocaleString()}`
      ).join("\n");
  ...
  app.listen(PORT, () => {
    console.log(`\n🚀 OHIMS running at http://localhost:${PORT}`);
